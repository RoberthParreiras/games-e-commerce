import Image from "next/image";
import Link from "next/link";

import { GameInfo } from "@/app/types/game";

export function GameCardAdmin({ game }: { game: GameInfo }) {
  return (
    <article className="w-full max-w-[400px] mx-auto flex flex-wrap justify-center hover:cursor-pointer">
      <Image
        src={game.image}
        alt={`${game.name} image`}
        width={400}
        height={400}
      />
      <div className="w-full flex flex-col text-center gap-10 mt-6">
        <h3 className="text-5xl">{game.name}</h3>
        <p className="text-2xl">{game.description}</p>
        <span className="text-3xl">{game.price}</span>
        <div className=" bg-[#DFD0B8] text-[#393E46] py-4 rounded-2xl hover:bg-[#948979]">
          <Link href={`/admin/editar/${game.id}`} className="text-5xl">
            Edit
          </Link>
        </div>
      </div>
    </article>
  );
}
