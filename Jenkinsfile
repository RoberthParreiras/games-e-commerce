pipeline {
    ...
    stages {
        ...
        stage('Start Dependencies for Testing') {
            steps {
                script {
                    // Create an empty .env file in the root project directory to satisfy docker-compose.
                    sh 'touch .env'

                    echo 'Cleaning up any old containers before starting...'
                    sh "docker compose ${env.TEST_COMPOSE_FILES} down --remove-orphans || true"

                    echo 'Starting database and dependency containers...'
                    // Use the TEST_COMPOSE_FILES to bring up the entire stack for testing.
                    sh "docker compose ${env.TEST_COMPOSE_FILES} up -d --no-build"

                    echo 'Listing running services...'
                    sh "docker compose ${env.TEST_COMPOSE_FILES} ps"
                }
            }
        }
        ...
        stage('Deploy to Production') {
            steps {
                script {
                    ...
                    sh "docker compose ${env.TEST_COMPOSE_FILES} up -d --build"
                }
            }
        }
    }
